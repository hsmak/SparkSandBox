...
object DateAddMonths extends App with MyContext {
  ...
  Seq(
    ...
    .select(
      ...
      add_months(to_date(col("Date"),"MM-dd-yyyy"),-3).as("sub_months"),
      date_add(to_date(col("Date"),"MM-dd-yyyy"),3).as("date_add"),
      date_add(to_date(col("Date"),"MM-dd-yyyy"),-3).as("date_sub"),
