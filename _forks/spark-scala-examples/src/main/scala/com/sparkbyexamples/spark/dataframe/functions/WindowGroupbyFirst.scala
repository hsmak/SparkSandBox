...
object WindowGroupbyFirst extends App with MyContext {
  ...
  spark.sql(
    """
      ...
      |      SELECT
      |         *,
      |         ROW_NUMBER() OVER(partition by department order by salary) as row -- Can't reference the new column in the where clause, hence the inner query
      |      FROM DF)
      |WHERE row == 1""".stripMargin).show(false)
  ...
  spark.sql(
    """
      ...
      |      SELECT
      |         *,
      |         ROW_NUMBER() OVER(partition by department order by salary DESC) as row -- Can't reference the new column in the where clause, hence the inner query
      |      FROM DF)
      |WHERE row == 1""".stripMargin).show(false)
