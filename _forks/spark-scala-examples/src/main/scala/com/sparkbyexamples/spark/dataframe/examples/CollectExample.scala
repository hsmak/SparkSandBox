...
object CollectExample extends App with MyContext{
  ...
  val schema = StructType(Seq(
    StructField("name", StructType(Seq( // Notice the nested StructType as a StructField
  ...
  colData.foreach(row =>  {
    ...
    val fullName:Row = row.getStruct(0) //Index starts from zero <- notice how StructType is being retrieved
